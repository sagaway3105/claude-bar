...
> [!IMPORTANT]
> **v1.0.0〜v1.5.2 の zip を展開して使っている方へ**: 当時の配布 zip には余計なファイル（AppleDouble `._*`）が混入しており、展開方法によってはアプリの署名が壊れます。「開発元を検証できない」と出る・自動アップデートがいつまでも終わらない場合は [Issue #1 の復旧手順](https://github.com/sagaway3105/claude-bar/issues/1) をご覧ください（v1.5.3 以降の zip は問題ありません）。
...
1. **Claude Codeをインストール**（未導入の場合）: `curl -fsSL https://claude.ai/install.sh | bash`
...
4. ログイン完了後、ClaudeBarが次の更新（既定5分ごと、またはパネルの↻）で自動的に使用量を表示する
5. 初回にmacOSからフォルダ等へのアクセス許可の確認が表示されることがあるので「許可」を選ぶ（キーチェーンの許可ダイアログは出ません）
